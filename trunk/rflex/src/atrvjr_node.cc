...
float heading_distance, heading_angle = 5000; 
...
bool controller_pub2CallBack( simple::controller_pub2::Request &req, simple::controller_pub2::Response &res ) {
  if (!req.publish_active) 
		updateHeading = true;
  ROS_INFO("SERVICE");
  return true;
}
...
void ATRVJRNode::setOdomTheta(float a) {
    a_odo = a;
}
...
int main(int argc, char** argv) {
  ...
  float dt = .1;   //time to step each iteration (s)
    ...
    ATRVJRNode node;
    std::string port;
    node.n.param<std::string>("port", port, "/dev/ttyUSB1");
    ...
    //Service message for cone mode switching
    //ros::ServiceServer service = n.advertiseService("/controller_pub2", controller_pub2CallBack);
...
//---------------------------------------------------------------------------
     ...
     * ------------------------------------ */
      ...
      if (updateHeading) {
			  ...
			  pathSlope = node.getSlope(x, xDest, y, yDest);
		  } else {
        d = 0;
        msgOut.linear.x = 0;
        msgOut.angular.z = 0;
      ...
      updateHeading = false;
  	  while (d > .1 && ros::ok() && !updateHeading) {
cout <<xDest<<" "<<yDest<<"\n";
cin;
		    ...
		    xPath = node.getIntersectX(pathSlope, pathSlope+PI/2, xDest, x, yDest, y);
		    if (xPath > xDest)
