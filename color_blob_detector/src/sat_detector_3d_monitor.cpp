#include "color_blob_detector/sat_detector_3d_monitor.h"

namespace enc = sensor_msgs::image_encodings;
using namespace cv;

SatDetector3DMonitor::SatDetector3DMonitor(std::string node_name, ros::NodeHandle nh){
  ...
  if(nh_.getParam("attribute_names", attribute_list)){
	...
	ROS_INFO("Loaded attribute name: %s", temp_att_name.c_str());
	...
	for (int i = 0; i < (int)att_value_list.size(); i++) 
	  {
	    ...
	    ROS_INFO("Loaded attribute val: %4.3f", temp_val);
  ...
  if (display_image_) {
    ...
    createTrackbar("Mean Color", window_name_, &mean_color_, 255, NULL);
  ...
  pointcloud_sub_ = nh_.subscribe(source_identifier_, 1, &SatDetector3DMonitor::pointCloudCallback, this);
  ...
  timer_ = nh_.createTimer(ros::Duration(1.0 / update_rate_), &SatDetector3DMonitor::update, this);
  //register the goal and feedback callbacks
  ROS_INFO("Sat Monitor %s started", node_name_.c_str());
}

void SatDetector3DMonitor::getPointIndices()
{
...
void SatDetector3DMonitor::pointCloudCallback(const sensor_msgs::PointCloud2ConstPtr& msg)
{
  if(!new_data_ && updated_){    
    ...
    *sat_image_ = Mat::zeros(sat_image_->size(), CV_8UC1);
    planes[1].copyTo(*sat_image_);
...
void SatDetector3DMonitor::update(const ros::TimerEvent& e) {
  if(new_data_ && !updated_){

    updated_ = true;
    
    //saturation is 0 -> 255, hue is 0 -> 179
    int rangeMin = std::max(mean_color_ - window_size_, 0);
    int rangeMax = std::min(mean_color_ + window_size_, 255);
	    
    Size ksize = Size(2 * blur_kernel_ + 1,2 * blur_kernel_ + 1);
    GaussianBlur(*sat_image_, *sat_image_, ksize, -1, -1);
    //blur(*sat_image_, *sat_image_, ksize, Point(-1, -1), BORDER_REPLICATE);
    //dilate(*sat_image_, *sat_image_, Mat(), Point(-1, -1), BORDER_REPLICATE);	    
    
    *binary_image_ = Mat::zeros(sat_image_->size(), CV_8UC1);

    if((rangeMin >= 0) && (rangeMax <= 255)){
      inRange(*sat_image_, Scalar((double)((uchar)rangeMin)),Scalar((double)((uchar)rangeMax)), *binary_image_);
    }

    *threshold_image_ = Mat::zeros(sat_image_->size(), CV_8UC3);
    ...
    if(color_indices_->indices.size() > 0){
      ...
      *cluster_image_ = Mat::zeros(sat_image_->size(), CV_8UC1);
...
void SatDetector3DMonitor::satToRGB(float &r, float &g, float &b){
...
int main(int argc, char** argv) {
  ros::init(argc, argv, "sat_detector_monitor");
  ros::NodeHandle nh("~");
  
  SatDetector3DMonitor node(ros::this_node::getName(), nh);
  ros::spin();
