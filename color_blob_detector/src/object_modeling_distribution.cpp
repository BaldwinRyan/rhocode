...
ObjectModelingDistribution::ObjectModelingDistribution(std::string node_name, ros::NodeHandle nh){
	...
	rgb_sub_ = nh_.subscribe(source_identifier_, 1, &ObjectModelingDistribution::pointCloudCallback, this);
...
void ObjectModelingDistribution::pointCloudCallback(const sensor_msgs::ImageConstPtr& msg_ptr)
{
...
//hue_var_coeff_
	...
	if(is_computing_floor==false){		
		...
		if(log_count_==100){
			...
			fp_hue_hist=fopen("/home/scfay/Desktop/hue_hist.txt","w");
			fp_sat_hist=fopen("/home/scfay/Desktop/sat_hist.txt","w");
			fp_val_hist=fopen("/home/scfay/Desktop/val_hist.txt","w");			
			if (fp_hue_hist == NULL) {
...
/*void ObjectModelingDistribution::rgbCallback(const sensor_msgs::ImageConstPtr& msg_ptr) {
