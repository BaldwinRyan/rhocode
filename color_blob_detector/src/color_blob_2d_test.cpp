...
#include <color_blob_detector/histogram_info_new_tests.h>
//#define DEBUG
...
#define DEFAULT_MIN_CC_THRESHOLD 400
...
int init_window_size = 4;
int init_blur_kernel = 20;
int min_second = 5;
int max_second = 8;
...
//Set default parameters if they are not already defined
    ...
    uv_pub_ = n_.advertise<geometry_msgs::PointStamped>(uv_topic, 1);
    ...
    if (display_image_) {
      ...
      createTrackbar("Min_Ratio", color_topic, &min_second, 100, &on_trackbar);
      createTrackbar("Max_Ratio", color_topic, &max_second, 100, &on_trackbar);
  ...
  void getMoments() {   
	...
	if (((waiting_for_cone) && (moment.mu02/moment.mu20 > min_second)) && (moment.mu02/moment.mu20 < max_second)) {
		pos_x = moment.m10/moment.m00;
		pos_y = moment.m01/moment.m00;
		waiting_for_cone = false;
	//cout << second << "\n";
	}
      }
    }
  ...
  double getConeProb(vector <Mat> x, int i, int j){
	...
	double satProb;
	double intProb;
	//if (x[0].data[i*width+j]%30 == 0.0)
	//	hueProb = 1.0;
	//else
		hueProb = cone_hue[x[0].data[i*width+j]];
	//	(1.0-
	//	cone_hue[0]-
	//	cone_hue[30]-
	//	cone_hue[60]-
	...
	//	cone_hue[210]-
	//	cone_hue[240]);
	//if (x[1].data[i*width+j] == 0)
	//	satProb = 1.0;
	//else
		satProb = cone_sat[x[1].data[i*width+j]];
	//	(1.0- cone_sat[0]);
	//if (x[2].data[i*width+j] == 255)
	//	intProb == 1.0;
	//else
		intProb = cone_int[x[2].data[i*width+j]];
	//	(1.0- cone_int[255]);
  ...
  double getBackProb(vector <Mat> x, int i, int j){
	...
	double satProb;
	double intProb;
	//if (x[0].data[i*width+j]%30 == 0)
	//	hueProb = 1.0;
	//else
		hueProb = without_cone_hue[x[0].data[i*width+j]];
	//	(1.0-
	//	without_cone_hue[0]-
	//	without_cone_hue[30]-
	//	without_cone_hue[60]-
	//	without_cone_hue[90]-
	//	without_cone_hue[120]-
	//	without_cone_hue[150]-
	//	without_cone_hue[180]-
	//	without_cone_hue[210]-
	//	without_cone_hue[240]);
	//if (x[1].data[i*width+j] == 0)
	//	satProb = 1.0;
	//else
		satProb = without_cone_sat[x[1].data[i*width+j]];
	//	(1.0- without_cone_sat[0]);
	//if (x[2].data[i*width+j] == 255)
	//	intProb == 1.0;
	//else
		intProb = without_cone_int[x[2].data[i*width+j]];
	//		(1.0 - without_cone_int[255]);
