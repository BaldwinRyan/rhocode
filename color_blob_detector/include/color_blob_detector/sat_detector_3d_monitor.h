...
const std::string DEFAULT_SOURCE_IDENTIFIER = "/kinect/camera/rgb/points";
const std::string DEFAULT_OUTPUT_IDENTIFIER = "/kinect/camera/rgb/feature/sat/some_sat";
const std::string DEFAULT_MARKER_TOPIC = "/kinect/camera/rgb/feature/sat/some_sat/marker";
...
const int DEFAULT_MAX_CLUSTER_SIZE = 2000;
...
protected:
  ...
  boost::shared_ptr<cv::Mat> sat_image_;	      // input image -> sat space
...
public:
  ...
  SatDetector3DMonitor(std::string node_name, ros::NodeHandle nh);

  ~SatDetector3DMonitor() {
    this->clear();
  };
  ...
  void satToRGB(float &r, float &g, float &b);
